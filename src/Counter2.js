...
const Prop = () => {
    ...
    const timer = setInterval(() => {
      setCount((prevCount) => {
        ...
        if (newCount === 20 * inc) {
          ...
          inc++; // Increment inc when count reaches multiples of 20
        }
        return newCount;
      });
    }, 1000);
  ...
  const buttonColor = count % 20 === 0 ? 'bg-blue-500' : 'bg-orange-600';

  return (
    <div className="flex flex-col justify-center items-center min-h-screen p-4 sm:p-10">
      <h1 className="font-bold text-3xl p-4 sm:p-10 text-center">Counter 2</h1>
